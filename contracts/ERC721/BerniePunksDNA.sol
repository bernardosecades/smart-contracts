...
contract BerniePunksDNA {
    ...
    // @see https://docs.chain.link/docs/vrf/v2/introduction/ (for production)
    ...
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter)
        public
        pure // dont read and write in blockchain (no gas)
        returns (uint256)
    {
        uint256 combineParams = _tokenId + uint160(_minter); // rememeber address is hexadecimal and in integer are uint160
